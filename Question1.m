...
Y = x';
cvpt = cvpartition(size(Y,1),'holdout', 0.25);
TrainSet = Y(training(cvpt),:);
TestSet = Y(test(cvpt),:);

mdl = fitcnb(TrainSet, 1);

errRate = loss(mdl,TestSet);
disp(['The error rate is ' , num2str(errRate)])

predQual = predict(mdl,TestSet);
confMat = consusionmatr(TestSet,predQual);
disp('The confusion Matrix is q')
disp(confat)
